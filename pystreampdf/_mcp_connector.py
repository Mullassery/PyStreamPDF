...
class PDFProcessor:
    ...
    def start_mcp_connector(self, port: int = 8780) -> str:
        from pystreampdf._mcp_tools import PyStreamPDFMCPHandler, PyStreamPDFMCPTools
        self.mcp_connector = _MCPPDFConnector(pdf=self, port=port)
...
class _MCPPDFConnector(BaseMCPConnector):
    def __init__(self, pdf: PDFProcessor, port: int = 8780):
        super().__init__("PyStreamPDF", port=port)
        self.pdf = pdf
